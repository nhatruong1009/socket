...
namespace Client {
	...
	{
	public:
		System::String^ ip = gcnew String("");
	public:
		Ip(void)
		{
			std::ifstream fi("defaultip.txt");
			if (fi) {
				std::string k;
				fi >> k;
				ip = gcnew String(k.c_str());
				fi.close();
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->ClientSize = System::Drawing::Size(293, 228);
...
#pragma endregion
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (serverConnect(socket, a)) {
			std::ofstream fo("defaultip.txt");
			fo << a;
			fo.close();
			Form^ Login = gcnew Client::Login(socket);
			this->Hide();
			Login->ShowDialog();
			closesocket(socket);
			WSACleanup();
